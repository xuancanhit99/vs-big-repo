...
int main() {
	...
	rein:;
		...
		case 2: {
			...
			int check =  LinearS(a, n, skey);
			if (check == -1)
				cout << "Value not found" << endl;
			else
				cout << "Value " << skey << " is present at index i = " << check << endl;
			break;
		}
		...
		case 3: {
			...
			int check = FindI(a, n, sfirst);
			if (check == -1)
				cout << "Value not found" << endl;
			else
				cout << "Index of the first occurrence i = " << check << endl;
			break;
		}
		case 4: {
			//6. Найти количество вхождений заданного значения в массиве (поиск всех вхождений).
			cout << "Enter value to search: "; cin >> stimes;
			int check = NumOfOcc(a, n, stimes);
			if (check == -1)
				cout << "Value not found" << endl;
			else
				cout << "The number of occurrences: " << check << endl;
			break;
		}
		case 5: {
