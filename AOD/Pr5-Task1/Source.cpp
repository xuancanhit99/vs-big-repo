...
#include <iomanip>
// ????????? ??? ??????????? ??????????? ???? ????? ????? ?????? ? ?????????????? ????????? ?????? ???????? ? ??????????????? / ????????????????? ? ?????????? ?????
// Program to shortest path between two nodes using the Floyd Warshall Algorithm in a directed/undirected and weighted graph  
using namespace std;
...
void InputDirected(vector<vector<int>>& graph) {
    int r, c, val, connect;
...
void InputUndirected(vector<vector<int>>& graph) {
    int r, c, val, connect;
    cout << "Enter number of connections you want: "; cin >> connect;
    for (int i = 0; i < connect; i++) {
        cout << "Enter vertex src, dest and weighted graph: "; cin >> r >> c >> val;
...
int main() {
    ...
    cout << "Program find the shortest path between two nodes using the" << endl
         << "Floyd Warshall Algorithm in a directed/undirectd and weighted graph." << endl << endl;
    int graphNum;
    cout << "1. Directed graph." << endl;
    cout << "2. Unirected grap." << endl;
    cout << "Please choose: "; cin >> graphNum;
    ...
    while (1) {
        ...
        case 2:
            ...
            cout << "Weight of the shortest path(distance) is: " << printDistance(src, dest) << endl;
            break;
        case 3:
            return 1;
        default:
