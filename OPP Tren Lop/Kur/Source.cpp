...
void Application::AddFullMatrix() {
	int num_int[10];
	string num_str;
	for (int i = 0; i < 10; i++) {
		cin >> num_str;
...
void Application::ReFirstUnit(int& i, int& j) {
	int out = 0;
	for (int k = 0; k < 10; k++) {
		for (int h = 0; h < 10; h++) {
			if (matrix[h][k] == 1) {
...
void Application::OutPut() {
	int i, j;
	Application::AddFullMatrix();
	Application::ReFirstUnit(i, j);
	Application::NextStep(i, j);
	Application::OutMatrix();
}

void Application::AllInOneMatrix() {
